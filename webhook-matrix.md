...
| **`IN_USE`** | **กำลังใช้งาน:** เริ่มมีการใช้งานดาต้าแล้ว | แสดงสถานะ "กำลังใช้งาน" แสดงแถบการใช้งาน Data (Usage Bar) เปิดรับแจ้งเตือนเน็ตใกล้หมด | Top-up, Suspend, Revoke |
...
| **`SUSPEND`** | **ถูกระงับ:** ถูกระงับการใช้งานชั่วคราว (สั่งโดยพาร์ทเนอร์หรือระบบ) | แสดงสถานะ "ถูกระงับชั่วคราว" (Paused) ปิดการเข้าถึงอินเทอร์เน็ต | Unsuspend, Revoke |
...
> 2. API ของ eSIMRocket (`/esim/topup`, `/esim/cancel`, `/esim/suspend` ฯลฯ) ไม่ได้มีการ Hard-block สถานะก่อนเรียกไปยัง Upstream เพื่อความยืดหยุ่นสูงสุด ดังนั้นการควบคุมว่า "สถานะไหนกดปุ่มอะไรได้บ้าง" ควรถูกบังคับ (Enforce) ที่ฝั่ง UI ของพาร์ทเนอร์ตามตารางด้านบน
...
| **พาร์ทเนอร์สั่งระงับ eSIM สำเร็จ** (Suspend) | `esim.suspended` | `ESIM_STATUS` | **`SUSPEND`** (Force ให้ 100%) |
| **พาร์ทเนอร์สั่งยกเลิกการระงับสำเร็จ** (Unsuspend) | `esim.unsuspended` | `ESIM_STATUS` | `IN_USE` |
...
```
*(กรณีที่เป็น `esim.unsuspended` จะมีฟิลด์พิเศษ `oldStatus` และ `newStatus` เพิ่มมาให้ใน data ด้วย)*
